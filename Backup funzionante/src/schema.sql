...
-- v15.8: un solo Jotter corrente persistente per owner
ALTER TABLE boards ADD COLUMN IF NOT EXISTS is_current BOOLEAN NOT NULL DEFAULT false;
WITH ranked AS (
 SELECT id,row_number() OVER(PARTITION BY owner_user_id ORDER BY content_updated_at DESC,created_at DESC,id DESC) rn
 FROM boards
)
UPDATE boards b SET is_current=(ranked.rn=1) FROM ranked WHERE ranked.id=b.id;
CREATE UNIQUE INDEX IF NOT EXISTS one_current_board_per_owner ON boards(owner_user_id) WHERE is_current=true;
CREATE INDEX IF NOT EXISTS boards_owner_current_idx ON boards(owner_user_id,is_current);
